...
public class RotateClick : MonoBehaviour {
        ...
        if (!isRotating)
        {
			...
			if (canRotate) {
				if (Input.GetKeyDown (KeyCode.LeftArrow)) {
					thisSpinning = StartCoroutine (RotateSlowly (Vector3.up));
				} else if (Input.GetKeyDown (KeyCode.RightArrow)) {
					thisSpinning = StartCoroutine (RotateSlowly (Vector3.down));
				} else if (Input.GetKeyDown (KeyCode.UpArrow)) {
					thisSpinning = StartCoroutine (RotateSlowly (Vector3.right));
				} else if (Input.GetKeyDown (KeyCode.DownArrow)) {
					thisSpinning = StartCoroutine (RotateSlowly (Vector3.left));
				}
			}
            if (Input.GetAxisRaw("RotateZ") != 0)
            {
				...
				disableRotation ();
                thisSpinning = StartCoroutine(RotateSlowly(rotationDirection));
            }
        }
    }

	IEnumerator RotateSlowly(Vector3 theDirection)
    {
	...
	public void StartRotating(Vector3 theDirection){
		disableRotation ();
		thisSpinning = StartCoroutine(RotateSlowly(theDirection));
	}
}
